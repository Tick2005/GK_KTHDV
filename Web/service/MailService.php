...
class MailService {
    ...
    private function getConfirmationEmailTemplate($transaction, $user, $student, $paidFeesDetails) {
        
        // 1. Khởi tạo biến để chứa HTML của các hàng chi tiết phí
        $feeRowsHtml = '';

        // 2. Vòng lặp PHP để tạo các hàng (TR)
        foreach ($paidFeesDetails as $fee) {
            $feeRowsHtml .= '
                <tr>
                    <td style="border-right:1px solid #eee;">' . htmlspecialchars($fee['semester'] ?? 'N/A') . '</td>
                    <td style="border-right:1px solid #eee;">' . htmlspecialchars($fee['school_year'] ?? 'N/A') . '</td>
                    <td style="text-align:right;">' . number_format($fee['amount'], 0, ',', '.') . '₫</td>
                </tr>';
        }

        // 3. Trả về toàn bộ HTML, chèn biến $feeRowsHtml vào vị trí thích hợp
        return '
            <!DOCTYPE html>
            <html lang="en">
            <head>
                    ...
                    <tr>
                        <td style="padding:30px;">
                            ...
                            <table width="100%" cellpadding="12" cellspacing="0" style="border-collapse:collapse; font-size:14px; color:#333; border:1px solid #eee;">
                                ...
                                <tr>
                                    <td style="font-weight:bold; border:1px solid #eee; padding:10px;">Paid Fee Items</td>
                                    <td style="border:1px solid #eee; padding:0;">
                                        <table width="100%" cellpadding="8" cellspacing="0" style="border-collapse:collapse; font-size:13px;">
                                            <thead style="background:#f0f0f0;">
                                                <tr>
                                                    <th style="border-right:1px solid #eee; text-align:left;">Semester</th>
                                                    <th style="border-right:1px solid #eee; text-align:left;">School Year</th>
                                ...
                                <tr style="background:#f9f9f9;">
                                    <td style="font-weight:bold; border:1px solid #eee;">Total Amount</td>
